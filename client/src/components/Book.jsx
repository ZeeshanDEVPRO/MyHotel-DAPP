...
const Book = ({ account, contractIns, connectContract }) => {
    ...
    const roomBooking = async (_hotelID, _roomID, _entry, _exit) => {
        ...
        try {
            const entryTimeUnix = Math.round(new Date(_entry).getTime());
            const exitTimeUnix = Math.round(new Date(_exit).getTime());
            console.log(entryTimeUnix, exitTimeUnix);

            // Fetch price from the contract in wei
            let priceAsValue = await contractIns.getFinalPrice(_hotelID, _roomID, entryTimeUnix, exitTimeUnix);
            console.log(parseInt(priceAsValue));
